...
public class VBrowser extends Browser implements ActionListener, ListSelectionListener, ASyncProcess {
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		//calculate sum SBBankStatementCreateFrom
		if(this.getAD_Browse_ID() == HardCoded.SBBankStatementCreateFrom_ID) {
			//calculate sum from selected payment amounts
			DefaultTableModel model = (DefaultTableModel) detail.getModel();
			int column = GetColumnAt.getColumnAt(detail , "PayAmt");
			BigDecimal total = new BigDecimal(0);
			
			for(int row = 0; row < model.getRowCount(); row ++) {
				IDColumn idColumn = (IDColumn) model.getValueAt(row, 0);
				if(idColumn != null && idColumn.isSelected()) {
					total = total.add((BigDecimal) model.getValueAt(row, column));
				}
			}
			
			statusBar.setStatusLine("SUM : " + total.doubleValue());
		}
