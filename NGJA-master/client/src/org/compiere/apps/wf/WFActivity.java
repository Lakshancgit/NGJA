 ...
 *         <li>BF[2992649] Issue in Workflow Activities when the records are
 *         ordered
 *         <li>
 *         https://sourceforge.net/tracker/?func=detail&aid=2992649&group_id=176962&atid=879332
 * @author Compiere - CarlosRuiz integrate code for table selection on workflow
 *         present at GPL version of Compiere 3.2.0
 */
public class WFActivity extends CPanel implements FormPanel, ActionListener, ListSelectionListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = 6917300855914216420L;

	private static final int MAX_ACTIVITIES_IN_LIST = MSysConfig.getIntValue("MAX_ACTIVITIES_IN_LIST", 200,
	...
	DefaultTableModel selTableModel = new DefaultTableModel(
			new String[] { "ID", Msg.translate(Env.getCtx(), "Priority"), Msg.translate(Env.getCtx(), "AD_WF_Node_ID"),
	...
	private CLabel lblSearch = new CLabel(Msg.getMsg(Env.getCtx(), "Search"));
	private CTextField txtSearch = new CTextField(20);
	
	private CLabel lblWorkflow = new CLabel(Msg.getMsg(Env.getCtx(), "Workflow"));
	private CComboBox cmbSearch = new CComboBox();
	...
	private void jbInit() throws Exception {
		...
		cmbSearch.addItem("Dealer/ Lapidarist/ Jewellery License"); // 0
		cmbSearch.addItem("Mining License"); // 1
		cmbSearch.addItem("Land"); // 2
		
		//
		int row = 0;
		selPane.setPreferredSize(new Dimension(width, 60));
		selPane.setMinimumSize(new Dimension(100, 60));
		centerPanel.add(selPane, new GridBagConstraints(0, row, 4, 1, 0.3, 0.3, GridBagConstraints.NORTHWEST,
				GridBagConstraints.BOTH, new Insets(5, 10, 5, 10), 0, 0));

		/*
		 * centerPanel.add (lNode, new GridBagConstraints (0, ++row, 1, 1, 0.0, 0.0,
		 * GridBagConstraints.NORTHEAST, GridBagConstraints.NONE, new Insets (5, 10, 5,
		 * 5), 0, 0)); centerPanel.add (fNode, new GridBagConstraints (1, row, 3, 2,
		 * 0.5, 0.0, GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL, new
		 * Insets (5, 0, 5, 10), 0, 0));
		 */

		/*
		 * centerPanel.add (lDesctiption, new GridBagConstraints (0, ++row, 1, 1, 0.0,
		 * 0.0, GridBagConstraints.NORTHEAST, GridBagConstraints.NONE, new Insets (5,
		 * 10, 5, 5), 0, 0)); centerPanel.add (fDescription, new GridBagConstraints (1,
		 * row, 3, 1, 0.0, 0.0, GridBagConstraints.NORTHWEST,
		 * GridBagConstraints.HORIZONTAL, new Insets (5, 0, 5, 10), 0, 0));
		 */

		/*
		 * centerPanel.add (lHelp, new GridBagConstraints (0, ++row, 1, 1, 0.0, 0.0,
		 * GridBagConstraints.NORTHEAST, GridBagConstraints.NONE, new Insets (2, 10, 5,
		 * 5), 0, 0)); centerPanel.add (fHelp, new GridBagConstraints (1, row, 3, 1,
		 * 0.0, 0.0, GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL, new
		 * Insets (2, 0, 5, 10), 0, 0));
		 */

		centerPanel.add(lHistory, new GridBagConstraints(0, ++row, 1, 1, 0.0, 0.0, GridBagConstraints.NORTHEAST,
				GridBagConstraints.NONE, new Insets(5, 10, 5, 5), 0, 0));
		centerPanel.add(fHistory, new GridBagConstraints(1, row, 3, 1, 0.5, 0.0, GridBagConstraints.NORTHWEST,
				GridBagConstraints.HORIZONTAL, new Insets(5, 0, 5, 10), 0, 0));

		centerPanel.add(lAnswer, new GridBagConstraints(0, ++row, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
				GridBagConstraints.NONE, new Insets(10, 10, 5, 5), 0, 0));
		centerPanel.add(answers, new GridBagConstraints(1, row, 2, 1, 0.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.HORIZONTAL, new Insets(10, 0, 5, 5), 0, 0));
		centerPanel.add(bZoom, new GridBagConstraints(3, row, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
				GridBagConstraints.NONE, new Insets(10, 0, 10, 10), 0, 0));

		centerPanel.add(lTextMsg, new GridBagConstraints(0, ++row, 1, 1, 0.0, 0.0, GridBagConstraints.NORTHEAST,
				GridBagConstraints.NONE, new Insets(5, 10, 5, 5), 0, 0));
		centerPanel.add(fTextMsg, new GridBagConstraints(1, row, 3, 1, 0.5, 0.0, GridBagConstraints.NORTHWEST,
				GridBagConstraints.HORIZONTAL, new Insets(5, 0, 5, 10), 0, 0));

		centerPanel.add(lForward, new GridBagConstraints(0, ++row, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
				GridBagConstraints.NONE, new Insets(10, 10, 5, 5), 0, 0));
		centerPanel.add(fForward, new GridBagConstraints(1, row, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.NONE, new Insets(10, 0, 5, 0), 0, 0));
		centerPanel.add(lOptional, new GridBagConstraints(2, row, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.NONE, new Insets(10, 5, 5, 5), 0, 0));
		centerPanel.add(bOK, new GridBagConstraints(3, row, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
				GridBagConstraints.NONE, new Insets(10, 5, 5, 10), 0, 0));
		
		
		//search
				centerPanel.add (lblSearch, new GridBagConstraints (0, row + 2, 1, 1, 0.0, 0.0, 
						GridBagConstraints.WEST, GridBagConstraints.NONE, 
						new Insets (5,	0, 5, 5), 0, 0));

				centerPanel.add (cmbSearch, new GridBagConstraints (1, row + 2, 1, 1, 0.0, 0.0, 
						GridBagConstraints.WEST, GridBagConstraints.NONE, 
						new Insets (10,	0, 5, 5), 0, 0));
				
				centerPanel.add (txtSearch, new GridBagConstraints (2, row + 2, 1, 1, 0.0, 0.0, 
					GridBagConstraints.WEST, GridBagConstraints.NONE, 
	...
	public int getActivitiesCount() {
		...
		String sql = "SELECT count(*) FROM AD_WF_Activity a " + "WHERE a.Processed='N' AND a.WFState='OS' AND ("
		// Owner of Activity
				+ " a.AD_User_ID=?" // #1
	...
	public int loadActivities() {
		...
		try {
			...
			while (rs.next()) {
				...
				if (activity.getAD_Workflow_ID() == 1000030) {
					StringBuffer sqlFileNo = new StringBuffer("SELECT (SELECT filenominingcustomer from c_invoice where C_Invoice_ID=c_GemMining.C_Invoice_ID) from c_GemMining where C_GemMining_ID= ?");
					Summary = activity.getSummary()+" File No - " + DB.getSQLValueString(null, sqlFileNo.toString(), activity.getRecord_ID());
				}else if (activity.getAD_Workflow_ID() == 1000036) {
					StringBuffer sqlFileNo = new StringBuffer("SELECT (SELECT filenominingcustomer from c_invoice where C_Invoice_ID=C_LandDetails.C_Invoice_ID) from C_LandDetails where C_LandDetails_ID= ?");
					Summary = activity.getSummary()+" File No - " + DB.getSQLValueString(null, sqlFileNo.toString(), activity.getRecord_ID());
	...
	public void actionPerformed(ActionEvent e) {
		...
		//	ADialog.error(m_WindowNo, this,"Please process the application, before proceed the workflow!");
		...
		//Search
			...
			//search workflows only available for Student Member Upgrade and converssion workflows
			if(txtSearch.getText() != null && txtSearch.getText().length() > 0){
				
				String val = txtSearch.getText().toUpperCase();
				
				if(cmbSearch.getSelectedIndex() == 0){ // student member registration
					String sql = "SELECT a.* , ad_user.name  FROM AD_WF_Activity a , ad_user , d_dealerlicense DL "
							+ "WHERE a.createdby = ad_user.ad_user_id AND a.Processed='N'  "
							+ "AND DL.d_dealerlicense_ID = 	A.record_id  AND "
							+ "DL.documentno LIKE  '"+val+"%'";
					this.search(sql);
					
				}else if(cmbSearch.getSelectedIndex() == 1){
					String sql = "SELECT a.* , ad_user.name  FROM AD_WF_Activity a , ad_user , c_gemmining GM "
						+ " WHERE a.createdby = ad_user.ad_user_id AND a.Processed='N' "
						+ " AND GM.c_gemmining_ID= A.record_id "
						+ " AND GM.documentno LIKE '"+val+"%'";
					this.search(sql);
					
				}else if(cmbSearch.getSelectedIndex() == 2){
					String sql = "SELECT a.* , ad_user.name  FROM AD_WF_Activity a , ad_user , C_LandDetails LD "
							+ " WHERE a.createdby = ad_user.ad_user_id AND a.Processed='N' "
							+ " AND LD.C_LandDetails_ID= A.record_id "
							+ " AND LD.documentno LIKE '"+val+"%'";
						this.search(sql);
	...
	private void search(String adtionalSearchSql){
		resetDisplay(null);
		while (selTableModel.getRowCount() > 0)
			selTableModel.removeRow(0);	
		long start = System.currentTimeMillis();
		...
		ArrayList<MWFActivity> list = new ArrayList<MWFActivity>();
		String sql = adtionalSearchSql 
			+ " AND a.WFState='OS' AND ("
			//	Owner of Activity
			+ " a.AD_User_ID=?"	//	#1
			//	Invoker (if no invoker = all)
			+ " OR EXISTS (SELECT * FROM AD_WF_Responsible r WHERE a.AD_WF_Responsible_ID=r.AD_WF_Responsible_ID"
	...
	private void cmd_button() {
		...
		if (MWFNode.ACTION_UserWindow.equals(node.getAction())) {
			String sql = "";
			// set unprocessed document
			if (m_activity.getPO().get_TableName().equalsIgnoreCase(MBPartner.Table_Name))
				sql = "UPDATE C_BPARTNER SET PROCESSED = 'N' , IsActive = 'Y'  WHERE C_BPARTNER_ID = ?";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MProduct.Table_Name))
				sql = "UPDATE M_Product SET PROCESSED = 'N' , IsActive = 'Y' WHERE M_Product_ID = ?";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MPayment.Table_Name))
				sql = "UPDATE C_Payment SET PROCESSED = 'N',IsActive = 'Y'  WHERE C_Payment_ID = ?  AND docStatus not in('VO', 'CO') OR DocStatus='IP'";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MOrder.Table_Name))
				sql = "UPDATE C_Order SET IsActive = 'Y' , SendForApproval = 'N' WHERE C_Order_ID = ?";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MRMA.Table_Name))
				sql = "UPDATE M_RMA SET IsActive = 'Y' , SendForApproval = 'N' WHERE M_RMA_ID = ?";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCColorCard.Table_Name))
				sql = "UPDATE C_ColorCard SET processed = 'N' WHERE C_ColorCard_ID = ?";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MDDealerLicense.Table_Name))
				sql = "UPDATE D_DealerLicense SET processed = 'N' WHERE D_DealerLicense_ID = ?";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCGemMining.Table_Name))
				sql = "UPDATE C_GemMining SET processed = 'N' WHERE   printapproval NOT IN ('Approved') and C_GemMining_ID = ? ";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCAuctionLicense.Table_Name))
				sql = "UPDATE C_AuctionLicense SET processed = 'N' WHERE C_AuctionLicense_ID = ? AND AuctionLicesneStatus not in('A')";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCLandDetails.Table_Name))
				sql = "UPDATE C_LandDetails SET processed = 'N' WHERE C_LandDetails_ID = ?  AND Applicationstatus not in ('RLE')";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCBackhoeService.Table_Name))
				sql = "UPDATE C_BackhoeService SET processed = 'N' WHERE C_BackhoeService_ID = ? AND LicenseStatus<>'A'";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCExportNGJA.Table_Name))
				sql = "UPDATE C_ExportNGJA SET processed = 'N' WHERE C_ExportNGJA_ID = ? ";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCMiningLicenseUpgrade.Table_Name))
				sql = "UPDATE C_MiningLicenseUpgrade SET processed = 'N' WHERE C_MiningLicenseUpgrade_ID = ? AND Status<>'U' ";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCDepositRelease.Table_Name))
				sql = "UPDATE C_DepositRelease SET processed = 'N' WHERE C_DepositRelease_ID = ? AND DepositReleaseStatus<>'CL'";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCImportNGA.Table_Name))
				sql = "UPDATE C_ImportNGA SET processed = 'N' WHERE C_ImportNGA_ID = ? ";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MExpReturnSale.Table_Name))
				sql = "UPDATE ExpReturnSale SET processed = 'N' WHERE ExpReturnSale_ID = ? ";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCBackhoeServiceFee.Table_Name))
				sql = "UPDATE C_BackhoeServiceFee SET processed = 'N' WHERE C_BackhoeServiceFee_ID = ? ";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCAuctionLicenseFinal.Table_Name))
				sql = "UPDATE C_AuctionLicenseFinal SET processed = 'N' WHERE C_AuctionLicenseFinal_ID = ? ";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MCLandOfShares.Table_Name))
				sql = "UPDATE C_LandOfShares SET processed = 'N' WHERE C_LandOfShares_ID = ? ";

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MInvoice.Table_Name)) {
				sql = "UPDATE C_Invoice SET IsActive = 'Y' WHERE C_Invoice_ID = ?";
			}
			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MMovement.Table_Name)) {
				sql = "UPDATE m_movement SET processed = 'N' WHERE m_movement_ID = ?";
			}

			else if (m_activity.getPO().get_TableName().equalsIgnoreCase(MDDOrder.Table_Name))
				sql = "UPDATE DD_Order SET processed = 'N' WHERE DD_Order_ID = ?";

			if (sql.length() > 0) {
				DB.executeUpdate(sql, m_activity.getRecord_ID(), m_activity.get_TrxName());
			}
	...
	private void cmd_OK() {
		...
		trx.close();
		this.setCursor(Cursor.getDefaultCursor());
		ADialog.info(m_WindowNo, this, "WorkflowResult", Msg.parseTranslation(Env.getCtx(), "@AD_WF_Node_ID@") + " : "
				+ m_activity.getNodeName() + " -> " + m_activity.getTextMsg() == null ? "" : m_activity.getTextMsg());
