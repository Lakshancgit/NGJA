...
import org.topra.modelvalidator.PayementCheckNo;
		...
		getSelectionKeys().stream().forEach( key -> {
			...
			if (currencyId==313) {
				if (!payment.isReceipt()) {
					transactionAmount 	= payment.getPayAmt().negate();
				}else {
					transactionAmount 	= payment.getPayAmt();
				}
				 
				 System.out.println("LKR");
			}else {
				 transactionAmount 	= getSelectionAsBigDecimal(key, "P_ConvertedAmount");	//  5- Converted Amount	
			}
