...
public class MInvoice extends X_C_Invoice implements DocAction {
	...
	public static MInvoice copyFrom(MInvoice from, Timestamp dateDoc, Timestamp dateAcct, int C_DocTypeTarget_ID,
			boolean isSOTrx, boolean counter, boolean isReversal, String trxName, boolean setOrder) {
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		// Prevent Duplicating File NO - 8th Feb 2021 - Auther:Lakshan-topra
		if (this.getM_PriceList_ID() == 1000009 && newRecord && !isReversal()) {

			StringBuffer sqlFileNo = new StringBuffer(
					"SELECT  count(c_invoice_ID)  FROM c_invoice WHERE docstatus in ('CO', 'CL') AND FileNoMiningCustomer=? ");
			int b = DB.getSQLValue(get_TrxName(), sqlFileNo.toString(), this.get_ValueAsString("FileNoMiningCustomer"));
			if (b == 0) {
				return true;
			} else {
				throw new AdempiereException("File No cant be duplicated!");
			}
		}
	...
	private void explodeBOM() {
		...
		while (count != 0) {
			...
			for (int i = 0; i < lines.length; i++) {
				...
				/*
				 * MProductBOM[] boms = MProductBOM.getBOMLines (product); for (int j = 0; j <
				 * boms.length; j++) { MProductBOM bom = boms[j]; MInvoiceLine newLine = new
				 * MInvoiceLine (this); newLine.setLine (++lineNo); newLine.setM_Product_ID
				 * (bom.getProduct().getM_Product_ID(), bom.getProduct().getC_UOM_ID());
				 * newLine.setQty (line.getQtyInvoiced().multiply( bom.getBOMQty ())); //
				 * Invoiced/Entered if (bom.getDescription () != null) newLine.setDescription
	...
	public String completeIt() {
		...
		Arrays.stream(getLines(false)).filter(invoiceLine -> invoiceLine != null).forEach(invoiceLine -> {
			...
			if (invoiceLine.getC_OrderLine_ID() != 0) {
				if (isSOTrx() || invoiceLine.getM_Product_ID() == 0) {
					...
					if ((isSOTrx() && MDocType.DOCBASETYPE_ARInvoice.equals(docBaseType)
							&& invoiceLine.getQtyInvoiced().signum() > 0) // Quantity invoiced
							|| (isSOTrx() && MDocType.DOCBASETYPE_ARCreditMemo.equals(docBaseType)
									&& invoiceLine.getQtyInvoiced().signum() < 0)) // Revert AR Credit Memo
						orderLine.setQtyInvoiced(orderLine.getQtyInvoiced().add(invoiceLine.getQtyInvoiced().abs()));
					else if ((isSOTrx() && MDocType.DOCBASETYPE_ARInvoice.equals(docBaseType)
							&& invoiceLine.getQtyInvoiced().signum() < 0) // Revert Invoiced
							|| (isSOTrx() && MDocType.DOCBASETYPE_ARCreditMemo.equals(docBaseType)
									&& invoiceLine.getQtyInvoiced().signum() > 0)) // AR Credit Memo
						orderLine.setQtyInvoiced(
								orderLine.getQtyInvoiced().add(invoiceLine.getQtyInvoiced().abs().negate()));
					/*
					 * else if ((!isSOTrx() && MDocType.DOCBASETYPE_APInvoice.equals(docBaseType) &&
					 * invoiceLine.getQtyInvoiced().signum() > 0) // Vendor Receipt || (!isSOTrx()
					 * && MDocType.DOCBASETYPE_APCreditMemo.equals(docBaseType) &&
					 * invoiceLine.getQtyInvoiced().signum() < 0)) // Revert Return Vendor
					 * orderLine.setQtyInvoiced(orderLine.getQtyInvoiced().add(invoiceLine.
					 * getQtyInvoiced().abs()); else if ((!isSOTrx() &&
					 * MDocType.DOCBASETYPE_APInvoice.equals(docBaseType) &&
					 * invoiceLine.getQtyInvoiced().signum() < 0) // Revert Vendor Receipt ||
					 * (!isSOTrx() && MDocType.DOCBASETYPE_APCreditMemo.equals(docBaseType) &&
					 * invoiceLine.getQtyInvoiced().signum() > 0)) // Return Vendor
					 * orderLine.setQtyInvoiced(orderLine.getQtyInvoiced().add(invoiceLine.
					 * getQtyInvoiced().abs()));
					 */
					orderLine.saveEx();
					/*
					 * if (!orderLine.save(get_TrxName())) { m_processMsg =
					 * "Could not update Order Line"; return DocAction.STATUS_Invalid; }
					 */
				}
				// Order Invoiced Qty updated via Matching Inv-PO
				else if (!isSOTrx() && invoiceLine.getM_Product_ID() != 0 && !isReversal()) {
					// MatchPO is created also from MInOut when Invoice exists before Shipment
					BigDecimal matchQty = invoiceLine.getQtyInvoiced();
					MMatchPO matchPO = MMatchPO.create(invoiceLine, null, getDateInvoiced(), matchQty);
					boolean isNewMatchPO = false;
					if (matchPO.get_ID() == 0)
						isNewMatchPO = true;
					matchPO.saveEx();
					/*
					 * if (!matchPO.save(get_TrxName())) { m_processMsg =
			...
			if (invoiceLine.getM_RMALine_ID() != 0) {
				...
				rmaLine.saveEx();
				/*
				 * if (!rmaLine.save(get_TrxName())) { m_processMsg =
			...
			if (!isSOTrx() && invoiceLine.getM_InOutLine_ID() != 0 && invoiceLine.getM_Product_ID() != 0
					&& !isReversal()) {
				 ...
				 * CLogger.retrieveErrorString("Could not create Invoice Matching"); return
...
//		MOrgInfo counterOrgInfo = MOrgInfo.get(getCtx(), counterAD_Org_ID);
		...
		// Deep Copy
		MInvoice counter = copyFrom(this, getDateInvoiced(), getDateAcct(), C_DocTypeTarget_ID, !isSOTrx(), true, false,
				get_TrxName(), true);
		//
	...
	public boolean reverseCorrectIt() {
		...
		// Deep Copy
		MInvoice reversal = copyFrom(this, getDateInvoiced(), getDateAcct(), getC_DocType_ID(), isSOTrx(), false, true,
				get_TrxName(), true);
		if (reversal == null) {
