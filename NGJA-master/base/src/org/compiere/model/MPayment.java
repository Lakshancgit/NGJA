	...
	implements DocAction, ProcessCall
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (getC_DocType_ID()==1000008 && getC_Invoice_ID() != 0) {
			MInvoice inv = new MInvoice(getCtx(), getC_Invoice_ID(), get_TrxName());
...
*/
	...
	public String completeIt()
	{
		...
		if ( isCashTrx() && !MSysConfig.getBooleanValue("CASH_AS_PAYMENT", true , getAD_Client_ID())) {
			...
			//When selecting a charge in cash payment, It should be selected the particular charge in cash journal when the cash payment is completed.
			//Note that selecting a charge in cash payment is optional. The above scenario is only applicable for cash type payments with a selected charge.
			
			if(this.getTenderType().equals(MPayment.TENDERTYPE_Cash) && this.getC_Charge_ID() != 0){
				cl.setCashType( X_C_CashLine.CASHTYPE_Charge);
				cl.setC_Charge_ID(this.getC_Charge_ID());
			}else if(this.getTenderType().equalsIgnoreCase(MPayment.TENDERTYPE_Cash) && this.getC_Invoice_ID() != 0){
				cl.setCashType( X_C_CashLine.CASHTYPE_GeneralReceipts);
				cl.setC_Invoice_ID(this.getC_Invoice_ID());
			}else {
				if(this.isReceipt())
					cl.setCashType( X_C_CashLine.CASHTYPE_GeneralReceipts);
				else
			...
			cl.setAD_Org_ID(this.getAD_Org_ID());
